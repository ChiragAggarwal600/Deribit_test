...
void fetchAndStreamOrderbook(WebSocketServer& wsServer) {
    while (running) {
        try {
            // Fetch orderbook from Deribit API
            auto sentTime = std::chrono::high_resolution_clock::now().time_since_epoch().count();
            for(const auto& symbol : subscribedSymbols) {
                auto start = std::chrono::high_resolution_clock::now();
                std::string orderbookResponse = UtilityNamespace::getInstrumentOrderbook(symbol);
                auto orderbookJson = nlohmann::json::parse(orderbookResponse);
                wsServer.sendOrderbookUpdate(symbol, orderbookJson);
                auto end = std::chrono::high_resolution_clock::now();
                auto marketDataProcessingLatency = std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count();
            ...
            auto propagationDelay = currentTime - sentTime;
            std::cout << "WebSocket Message Propagation Delay: " << propagationDelay << " ns" << std::endl;
...
void on_message(websocketpp::client<websocketpp::config::asio>* c, websocketpp::connection_hdl hdl, websocketpp::client<websocketpp::config::asio>::message_ptr msg) {
    try {
...
void subscribeToSymbols(websocketpp::client<websocketpp::config::asio>& client, websocketpp::connection_hdl hdl) {
...
int main() {
    ...
    try {
        ...
        // Place an order
        auto start = std::chrono::high_resolution_clock::now();
        std::string response1 = orderManager.placeOrder("BTC-PERPETUAL", "buy", 10.0, 97000.0);
        auto end = std::chrono::high_resolution_clock::now();
        ...
        // Modify an order
        std::string response3 = orderManager.modifyOrder(orderId, 10.0, 96000.0);
